...
- **01 Core**
  - `Core.Api` — ASP.NET Core Web API, JWT bearer auth (access + rotating refresh tokens), Quartz scheduled jobs, Swagger. Controllers:
    - `Ai/AiTrainingWorkOutIntensityCsvController` — export/import the WorkoutIntensity training-data CSV (see [Data flow](#data-flow-high-level) and `sources/FitnessAi/docs/AIReadme.en.md`)
...
- **02 Logic**
  - `Logic.Ai` — AI/training-data logic:
    - `Csv/` — generic CSV read/write infrastructure keyed by `AiModelTypeEnum` (reused as the CSV "AiType" selector): `IColumnDefinitionFactory`/`ColumnDefinitionFactory`, `CsvHeaderValidator`, `ICsvModelLoader<T>`/`CsvModelLoader<T>`, `ICsvModelCreator<T>`/`CsvModelCreator<T>`, `ICsvRowMapper<T>` (the per-model extension point), `IAiTrainingDataFileService`/`AiTrainingDataFileService` (persistence for `AiTrainingDataFileTable`) — see `sources/FitnessAi/docs/AIReadme.en.md`
    - `Csv/ModelMapper/WorkoutIntensityCsvRowMapper` — the only concrete `ICsvRowMapper<T>` today, backing the WorkoutIntensity training-data CSV
    - `Csv/Services/AiWorkOutIntensityTrainingFileService` (`IAiWorkOutIntensityTrainingFileService`) — load-unlabeled-rows / accept-labeled-upload / get-existing-file, all merged by `DataKey` (never duplicated, labeled rows always preferred/kept)
...
- **03 Data**
  ...
  - `Data.Database` — EF Core `DbContext`, entities, and migrations (MySQL via `MySql.EntityFrameworkCore`). Entity groups:
    ...
    - `Entities/Ai` — `AiTrainingDataFileEntity` (`AiTrainingDataFileTable`): one row per `AiModelTypeEnum`, holding the current training-data CSV (`Csv` byte[]) and an `IsUpdated` flag — generic storage backing `Logic.Ai/Csv/IAiTrainingDataFileService`
    - `Entities/Scheduler` — `ScheduledJobEntity` for Quartz-backed jobs
    - `Entities/Settings` — `SettingsEntity`, `AISettingsEntity`
- **04 Shared** — `Shared.Enums` (including `Ai/AiModelTypeEnum`, reused as the CSV "AiType" selector — see `Logic.Ai/Csv/`; and `Ai/WorkoutIntensityEnum`), `Shared.Interfaces`, `Shared.Models` — enums, contracts, and DTOs shared across API, logic, and data layers (also paired 1:1 with the mobile client's TS import types — see [Client/backend model sync](#clientbackend-model-sync))
...
3. For workout intensity specifically, `Logic.Ai.Csv.Services.AiWorkOutIntensityTrainingFileService` exports not-yet-labeled `HealthConnectTrainingDataTable` rows as a CSV (`Ai/AiTrainingWorkOutIntensityCsvController`); an external process/human fills in a `Label` per row; the labeled CSV is uploaded back and merged (by `DataKey`, labeled rows always kept) into a single stored file in `AiTrainingDataFileTable`. There is currently no in-process model training/prediction step — see `sources/FitnessAi/docs/AIReadme.en.md` (or `AIReadme.de.md` for German) for the full write-up, including why this replaced an earlier in-process ML.NET pipeline.
