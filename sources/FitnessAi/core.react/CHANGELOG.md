...
- Erstellen Sie ein React-Projekt mit create-vite: `npm init --yes vite@latest "core.react" -- --template=react-ts  --no-rolldown --no-immediate --eslint`..
- `vite.config.ts` wird mit dem Port aktualisiert.
- Projektdatei (`core.react.esproj`) erstellen.
