...
import type { UpdateUserProfileRequest, UserProfile } from "../userProfile.types";
...
const toDisplayValue = (value: number | null): string => (value === null ? "" : String(value));
const toRequestValue = (value: string): number | null => (value.trim() === "" ? null : Number(value));
...
export function UserProfileForm({ profile, onSave, isSaving, saveError, saveSuccess }: UserProfileFormProps) {
  const { getResource } = useI18n();
  const formValues = useMemo(() => toFormValues(profile), [profile]);
  const { useField, useFormState, useModel, handleSubmit } = useForm<ProfileFormValues>({
  ...
  return (
    <Paper sx={{ p: 3 }}>
      <Stack component="form" onSubmit={handleSubmit} sx={{ gap: 2 }}>
        <Typography variant="h6">{getResource("common.labelProfile")}</Typography>

        <Stack direction="row" sx={{ gap: 2, flexWrap: "wrap" }}>
          <AppTextField label={getResource("common.firstName")} {...firstName} required sx={{ minWidth: 220 }} />
          <AppTextField label={getResource("common.lastName")} {...lastName} required sx={{ minWidth: 220 }} />
        </Stack>

        <AppTextField label={getResource("common.email")} value={profile.email} disabled sx={{ maxWidth: 460 }} />
        ...
        <Stack direction="row" sx={{ gap: 2, flexWrap: "wrap" }}>
          <AppTextField type="number" label={getResource("common.labelHeight")} {...height} sx={{ minWidth: 160 }} />
          <AppTextField type="number" label={getResource("common.labelWeight")} {...weight} sx={{ minWidth: 160 }} />
          <AppTextField
            type="number"
        ...
        <Stack direction="row" sx={{ gap: 2, flexWrap: "wrap" }}>
          <AppTextField type="number" label={getResource("common.labelWaist")} {...waist} sx={{ minWidth: 160 }} />
          <AppTextField type="number" label={getResource("common.labelAbdomen")} {...abdomen} sx={{ minWidth: 160 }} />
          <AppTextField
            type="number"
        ...
        <AppTextField label={getResource("common.labelBmi")} value={bmi > 0 ? bmi.toFixed(1) : "-"} disabled />
        ...
        {saveSuccess && <Alert severity="success">{getResource("common.profileUpdateSuccess")}</Alert>}

        <AppButton type="submit" disabled={isSaving || !isDirty} sx={{ alignSelf: "flex-start" }}>
